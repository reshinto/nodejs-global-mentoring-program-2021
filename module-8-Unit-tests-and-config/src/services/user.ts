...
export const checkUser = async ({ username, password }: {username: string; password: string}): Promise<UserAttributes[] | unknown[]> => {
  try {
    const users = await getNonDeletedUsers() as unknown as UserAttributes[];
    return users.filter((user: UserAttributes) => user.login === username && user.password === password);
  } catch (error: unknown) {
    const err = new Error();
    logError(err, error);
    return [error];
